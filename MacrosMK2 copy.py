...
class window(QMainWindow):
    def __init__(self):
        ...
        self.temp = self.findChild(QPushButton, 'pushButton')
        ...
        if running == False:
            ...
            self.temp.clicked.connect(self.bbutton)
    ...
    def bbutton(self):
        currentIndex = self.listWidget.currentRow()
        text, ok = QInputDialog.getText(self, 'Wait for Input', 'Type key that will continue')
        if ok and text is not None:
            self.bu2(text, '', currentIndex)
            commandList.insert(currentIndex+1, ['waitForInput', text, ''])
            print(commandList, currentIndex)
    
    def bu2(self, text, y, currentIndex):
        self.listWidget.insertItem(currentIndex+1, 'Wait for: '+ text)
        self.listWidget.setCurrentRow(currentIndex+1)
